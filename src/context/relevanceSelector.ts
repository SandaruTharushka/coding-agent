...
const ENTRY_PATTERNS = ['index', 'main', 'cli', 'app', 'server']
...
function scoreRecord(record: FileRecord, taskWords: string[]): ScoredFile {
  ...
  const relLower = record.relativePath.toLowerCase()
  const basename = path.basename(relLower, record.extension)
  const dirParts = path.dirname(relLower).split(path.sep)

  // Filename match (high weight — very specific signal)
