...
import type { Page, AgentStatus, LogEntry, GitStatus } from '../types'
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'ADD_LOG':
      return { ...state, logs: [...state.logs.slice(-2000), action.entry] }
    case 'CLEAR_LOGS':
      return { ...state, logs: [] }
    case 'SET_GIT_STATUS':
      return { ...state, gitStatus: action.status }
    case 'SET_SESSION':
      return { ...state, lastSessionId: action.sessionId }
    default:
      return state
  }
}
